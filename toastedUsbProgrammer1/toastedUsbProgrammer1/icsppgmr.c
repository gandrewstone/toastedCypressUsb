...
uint8_t initChipAccess(unsigned char* devId)
{
  ...
  uint8_t j,k;
  //bitClr(UsbLed);
  for(i=0;((i<8)&&(sig==0));i++)  /* search for the fastest working clock divider */
    {
    spiInit(i);
    ...
    if (ret) 
      {
	  unsigned long int priorsig=0;
      for(j=0;(j<10);j++)  // if it does not work consistently I need to go slower
        {
        sig = avrGetDeviceSig();
		//if (sig != 0)
		{
		...
		//if (sig == 0x303030) sig = 0;  // Bad value
		if ((devId[0] == 0x1E)||(devId[2]==0x1E))
		{
		if (priorsig==0)
		  {
#if 0		  
		  ...
		  {
		  if (sig!=priorsig) { sig=0; break; }  // Sorry not stable
		  //delay(1*mSec);
		  }
