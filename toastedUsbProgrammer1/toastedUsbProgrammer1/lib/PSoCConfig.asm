...
export LoadConfig_toastedUsbProgrammer1
export _LoadConfig_toastedUsbProgrammer1
export UnloadConfig_toastedUsbProgrammer1
export _UnloadConfig_toastedUsbProgrammer1
export ReloadConfig_toastedUsbProgrammer1
export _ReloadConfig_toastedUsbProgrammer1
export LoadConfig_serialPassThru
export _LoadConfig_serialPassThru
export UnloadConfig_serialPassThru
export _UnloadConfig_serialPassThru
...
_LoadConfigInit:
 LoadConfigInit:
	...
	mov		[Port_1_Data_SHADE], 12h

	lcall	LoadConfig_toastedUsbProgrammer1
	lcall	LoadConfigTBL_toastedUsbProgrammer1_Ordered
...
;---------------------------------------------------------------------------
; Load Configuration toastedUsbProgrammer1
;
;    Load configuration registers for toastedUsbProgrammer1.
...
;               CUR_PP
;
_LoadConfig_toastedUsbProgrammer1:
 LoadConfig_toastedUsbProgrammer1:
    RAM_PROLOGUE RAM_USE_CLASS_4
	M8C_SetBank1
    ...
    asr     a                       ; Store in carry flag
                                    ; Load bank 0 table:
    mov     A, >LoadConfigTBL_toastedUsbProgrammer1_Bank0
    mov     X, <LoadConfigTBL_toastedUsbProgrammer1_Bank0
    lcall   LoadConfig              ; Load the bank 0 values

    mov     a, 1                    ; Specify bank 1
    asr     a                       ; Store in carry flag
                                    ; Load bank 1 table:
    mov     A, >LoadConfigTBL_toastedUsbProgrammer1_Bank1
    mov     X, <LoadConfigTBL_toastedUsbProgrammer1_Bank1
    lcall   LoadConfig              ; Load the bank 1 values

	RAM_SETPAGE_CUR >ACTIVE_CONFIG_STATUS
; set config active bit
	or		[ACTIVE_CONFIG_STATUS+toastedUsbProgrammer1_ADDR_OFF], toastedUsbProgrammer1_BIT
...
;---------------------------------------------------------------------------
; Reload Configuration toastedUsbProgrammer1
;
;    Reload configuration registers for toastedUsbProgrammer1.
;    IO Bank 0 registers a loaded first,then those in IO Bank 1.
;
;       INPUTS: None.
;      RETURNS: Nothing.
...
;               CUR_PP
;
_ReloadConfig_toastedUsbProgrammer1:
 ReloadConfig_toastedUsbProgrammer1:
    RAM_PROLOGUE RAM_USE_CLASS_4
    ...
    mov     A, >ReloadConfigTBL_toastedUsbProgrammer1_Bank0
    mov     X, <ReloadConfigTBL_toastedUsbProgrammer1_Bank0
    lcall   LoadConfig              ; Reload the bank 0 values

    mov     a, 1                    ; Specify bank 1
    asr     a                       ; Store in carry flag
                                    ; Bank 1 table address:
    mov     A, >ReloadConfigTBL_toastedUsbProgrammer1_Bank1
    mov     X, <ReloadConfigTBL_toastedUsbProgrammer1_Bank1
...
;---------------------------------------------------------------------------
; Unload Configuration toastedUsbProgrammer1
;
;    Reset configuration registers for toastedUsbProgrammer1
...
;               CUR_PP
;
_UnloadConfig_toastedUsbProgrammer1:
 UnloadConfig_toastedUsbProgrammer1:
    RAM_PROLOGUE RAM_USE_CLASS_4
    ...
    asr     a                       ; Store in carry flag
                                    ; Bank 0 table address:
    mov     A, >UnloadConfigTBL_toastedUsbProgrammer1_Bank0
    mov     X, <UnloadConfigTBL_toastedUsbProgrammer1_Bank0
    lcall   LoadConfig              ; Unload the bank 0 values

    mov     a, 1                    ; Specify bank 1
    asr     a                       ; Store in carry flag
                                    ; Bank 1 table address:
    mov     A, >UnloadConfigTBL_toastedUsbProgrammer1_Bank1
    mov     X, <UnloadConfigTBL_toastedUsbProgrammer1_Bank1
...
; clear config active bit
	RAM_SETPAGE_CUR >ACTIVE_CONFIG_STATUS
	and		[ACTIVE_CONFIG_STATUS+toastedUsbProgrammer1_ADDR_OFF], ~toastedUsbProgrammer1_BIT
    M8C_SetBank0                    ; Force return to bank 0
	pop		x
...
;---------------------------------------------------------------------------
; Load Configuration serialPassThru
;
;    Load configuration registers for serialPassThru.
;    IO Bank 0 registers a loaded first,then those in IO Bank 1.
;
;       INPUTS: None.
;      RETURNS: Nothing.
...
;               CUR_PP
;
_LoadConfig_serialPassThru:
 LoadConfig_serialPassThru:
    RAM_PROLOGUE RAM_USE_CLASS_4
	M8C_SetBank1
; writing SpiConfig register
	and		reg[29h], ~e5h
	or		reg[29h],  5h
; writing SpiControl register
	M8C_SetBank0
	and		reg[2bh], ~80h
; writing TimerConfig:0 register
	and		reg[b0h], ~ 4h
; writing TimerData0:0 register
	and		reg[b2h], ~ffh
    ...
    asr     a                       ; Store in carry flag
                                    ; Load bank 0 table:
    mov     A, >LoadConfigTBL_serialPassThru_Bank0
    mov     X, <LoadConfigTBL_serialPassThru_Bank0
    lcall   LoadConfig              ; Load the bank 0 values

    mov     a, 1                    ; Specify bank 1
    asr     a                       ; Store in carry flag
                                    ; Load bank 1 table:
    mov     A, >LoadConfigTBL_serialPassThru_Bank1
    mov     X, <LoadConfigTBL_serialPassThru_Bank1
    lcall   LoadConfig              ; Load the bank 1 values

	RAM_SETPAGE_CUR >ACTIVE_CONFIG_STATUS
; set config active bit
	or		[ACTIVE_CONFIG_STATUS+serialPassThru_ADDR_OFF], serialPassThru_BIT
    M8C_SetBank0                    ; Force return to bank 0
	pop		x
...
;---------------------------------------------------------------------------
; Unload Configuration serialPassThru
;
;    Reset configuration registers for serialPassThru
;    to their values as initially configured.
;    IO Bank 0 registers a loaded first,then those in IO Bank 1.
;
;       INPUTS: None.
...
;               CUR_PP
;
_UnloadConfig_serialPassThru:
 UnloadConfig_serialPassThru:
    RAM_PROLOGUE RAM_USE_CLASS_4
    ...
    asr     a                       ; Store in carry flag
                                    ; Bank 0 table address:
    mov     A, >UnloadConfigTBL_serialPassThru_Bank0
    mov     X, <UnloadConfigTBL_serialPassThru_Bank0
    lcall   LoadConfig              ; Unload the bank 0 values

    mov     a, 1                    ; Specify bank 1
    asr     a                       ; Store in carry flag
                                    ; Bank 1 table address:
    mov     A, >UnloadConfigTBL_serialPassThru_Bank1
    mov     X, <UnloadConfigTBL_serialPassThru_Bank1
    lcall   LoadConfig              ; Unload the bank 1 values
...
; clear config active bit
	RAM_SETPAGE_CUR >ACTIVE_CONFIG_STATUS
	and		[ACTIVE_CONFIG_STATUS+serialPassThru_ADDR_OFF], ~serialPassThru_BIT
    M8C_SetBank0                    ; Force return to bank 0
	pop		x
