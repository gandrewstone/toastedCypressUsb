...
USBFS_CB_d2h_std_ifc_10:
    ...
    cmp     A, 2h                      ; Valid interface number? (UM Parameter: NumInterfaces)
    jnc     USBFS_Not_Supported
...
;mykt_ukr    index   USBFS_USB_EPX_BIT_LOOKUP	   ; Find bit position for endpoint
    push X
    add     A, <USBFS_USB_EPX_BIT_LOOKUP           ; endpoint address LSB
    mov     X, >USBFS_USB_EPX_BIT_LOOKUP           ; endpoint address MSB
    swap    A, X
    adc     A, 0
    romx
    pop X
    ...
    xor     A, FFh
    and     [USBFS_EPXDataToggle], A ; Clear the data toggle for this endpoint
...
.EP_INT_ENABLE:
    jmp     .EP0IntEnable              ; Enable EP0 -- this jump should never be made
    jmp     .EP1IntEnable              ; Enable EP1
    jmp     .EP2IntEnable              ; Enable EP2
    jmp     .EP3IntEnable              ; Enable EP3
    jmp     .EP4IntEnable              ; Enable EP4
    jmp     .EP5IntEnable              ; Enable EP5
    jmp     .EP6IntEnable              ; Enable EP6
    jmp     .EP7IntEnable              ; Enable EP7
    jmp     .EP8IntEnable              ; Enable EP8

; Jump here to enable EP0 Interrupts
.EP0IntEnable:
    M8C_EnableIntMask USBFS_INT_REG1, USBFS_INT_EP0_MASK
;    jmp   .exit2 
    jmp     .cont                      ; Inspection of calling code shows that .EP0IntEnable is/should never be used
.EP1IntEnable:
    M8C_EnableIntMask USBFS_INT_REG1, USBFS_INT_EP1_MASK
    jmp     .cont
.EP2IntEnable:
    M8C_EnableIntMask USBFS_INT_REG1, USBFS_INT_EP2_MASK
    jmp     .cont
.EP3IntEnable:
    M8C_EnableIntMask USBFS_INT_REG1, USBFS_INT_EP3_MASK
    jmp     .cont
.EP4IntEnable:
    M8C_EnableIntMask USBFS_INT_REG2, USBFS_INT_EP4_MASK
	jmp     .cont
.EP5IntEnable:
    M8C_EnableIntMask USBFS_INT_REG2, USBFS_INT_EP5_MASK
	jmp     .cont
.EP6IntEnable:
    M8C_EnableIntMask USBFS_INT_REG2, USBFS_INT_EP6_MASK
	jmp     .cont
.EP7IntEnable:
    M8C_EnableIntMask USBFS_INT_REG2, USBFS_INT_EP7_MASK
	jmp     .cont
.EP8IntEnable:
    M8C_EnableIntMask USBFS_INT_REG2, USBFS_INT_EP8_MASK
	jmp     .cont
...
;	jmp   .exit2
.cont:
                                       ; this code should never be called to manage EP0
...
.exit1:
	M8C_SetBank0
.exit2:
;    mov   [X+USBFS_EPXDataToggle], 0   ; Clear all EP data toggles?  Is this right?
    ret
