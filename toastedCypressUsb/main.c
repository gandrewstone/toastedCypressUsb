 ...
 uint8_t serial2Spi(void)
{
  ...
  //pinMode(RedLedRegNum,RedLedMsk,PinModeOutput);
  //bitSet(RedLed);
  ...
  while (loop) 
    ...
    #if 0  // Outputs letters to the USB for debugging
    if (lasttick < tick)
    {
      SendSerialState();
      lasttick = tick + 0x4000UL;
...
{   
    ...
    bitSet(IcspReset); 
    //bitClr(IcspReset);    // Hold AVR in reset for now
  ...
  while (1)
    ...
    if ((usbCfg == USB_CONFIG_SERIAL)&&(usbGood))
      ...
      c = serial2Spi();
      usbGo();
      if ((c!=1)&&(usbCfg == USB_CONFIG_SERIAL)) bootloader();      
      }
    else
      {
      bitClr(BluLed);
